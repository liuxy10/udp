...
def main(in_port = "4444", out_port = "3333", vis = True, 
    ...
    env = RealObsFeature(reward_map, 
                         ...
                         thrust=0.02, # IMPORTANT: step size of param update
                         start = [0.5, 0.5], # make sure aligning with core.py
                         cfg = cfg,
                         in_port = in_port, out_port = out_port,
                         max_episode_steps=cfg.lspi_cfg.update_size * cfg.skip_num) # max episode steps before done signal
    ...
    for iter in range(n_iter):
        ...
        state_cur = env.receive_state( vis = True, wait = 1000)
        reward_cur = reward_map(state_cur, np.zeros(env.n_action))
        done_cur = False
        while env.counter < cfg.lspi_cfg.update_size * cfg.skip_num and not done_cur:
            if env.counter % env.cfg.skip_num == 0: # when using the state to update
                print(f"physical/learning step {env.counter}/{env.counter//env.cfg.skip_num}, current state: {state_cur}, current reward: {env.reward_map(state_cur, np.zeros(env.n_action))}")
                action = agent.predict(state_cur)
                # print(f"State: {state}, Action: {action}")
                state, reward, done, info = env.step(action) # update env with action and receive new state
                env.counter += 1
                env.memory.append(Sample(s=state_cur, a=action, r=reward_cur, s_=state, done=done))
                state_cur, reward_cur, done_cur = state, reward, done
                
                if done:
                    baseline.agent.save(os.path.join(env.cfg.policy_path, "online", "weight"), name="weights_online")
                    env.memory = env.memory[-cfg.lspi_cfg.memory_size:] # delete starting samples to avoid bias 
                ...
                state = env.receive_state( vis = True, wait = 1000)
                env.counter += 1
                
        
        env.save_memory(os.path.join(env.cfg.policy_path, "online", "memory"), name = "memory_online_iter_"+str(iter)+".csv")
                    
        # policy update 
        w = baseline.agent.weights.copy()
        baseline.load_memory(env.memory, vis = True)
        ...
        for it in range(10): # max 10 policy updates
            ...
            if np.linalg.norm(baseline.agent.weights - w) < 1e-4:
                break
            w = baseline.agent.weights.copy()
        baseline.agent.save(os.path.join(env.cfg.policy_path, "online", "weights"), name="weights_online_iter_"+str(iter))
